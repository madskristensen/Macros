# Danny History Archive

Archived on: 20260502T172031Z
...
### 2026-05-02 — PromptAsync uses an engine-to-VSIX prompt service seam

`Helpers.PromptAsync` lives in `Macros.Engine`, but the actual dialog must live in the WPF-enabled VSIX project. The clean split is an internal `IMacroPromptService` carried through `MacroGlobals` by `MacroPlayer`, so the engine stays UI-agnostic while runtime macros still get a themed Visual Studio dialog.

**Testing pattern:** keep unit tests in `Macros.Tests` at the seam by setting `Helpers.CurrentGlobals`, injecting a fake prompt service, and supplying a test-owned `JoinableTaskFactory` through `MacroGlobals.UiThreadFactory`. That lets `PromptAsync` behavior be verified without a hosted VS instance.

**Verification:** `dotnet build` and `dotnet test --no-build` from the repo root both pass after the change.
