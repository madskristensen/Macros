...
| **AfterCommand**  | `// @trigger AfterCommand {Command.Name}`  | Runs **after** the command completes. Queued — never blocks the IDE. Can observe success/failure via `Trigger.Payload["Success"]` (bool).                                                                                                                                                      |
